...
{
    public class DesignEnvironmentV1
    {
        ...
        public static Species Beech = new Species("beech",0.0001, 0.0020, 0.0041, 14000, 2280, 1160);
        public static Species Spruce = new Species("spruce",0.0001, 0.0019, 0.0036, 10000, 800, 450);

        public static Species PassiveMaterial = Spruce;

        public DesignEnvironmentV1(List<Panel> panels, List<StockPile> stockPiles)
        {
            Panels = panels;
        ...
        public void FlattenLists()
        {
            ...
            {
                panel.ID = panelID;
                RhinoApp.WriteLine(panelID.ToString());
        ...
        public void ApplyStock()
        {
            ...
            foreach(StockPile stockPile in StockPiles) boardDicts.Add(stockPile.Species.Name, new Tuple<int, List<PanelBoard>>(stockPile.BoardCount, new List<PanelBoard>()));
            ...
            foreach (string material in boardDicts.Keys)
            {
                ...
                foreach (PanelBoard board in radiusWeightOrder)
                {
                    ...
                    {
                        if (stockBoard.Species.Name != material | stockBoard.LengthAvailable < board.Length) continue;
